...
# E1.2 strict reproduction entrypoint (Option 2, v11.0.0).
#
# This is intentionally separate from reproduce_v10_1_late_time.sh:
# - late-time paper build remains canonical and unchanged
# - E1.2 strict (CHW2018 vector+cov) outputs go to a separate results dir

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
V101_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

RESULTS_DIR="${V101_DIR}/results/late_time_fit_cmb_e12_strict"
FIG_DIR="${RESULTS_DIR}/figures"

PAPER_ASSETS_DIR="${V101_DIR}/paper_assets_cmb_e12_strict"
