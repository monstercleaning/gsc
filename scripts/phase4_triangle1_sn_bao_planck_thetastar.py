#!/usr/bin/env python3
"""Deterministic Triangle-1 SN+BAO+Planck-theta* epsilon posterior (Phase-4 M157 / 4B.5)."""
...
from gsc.datasets.bao import BAOBlock1D, BAOBlock2D, BAOBlockND, BAODataset, D_H, D_V_flat  # noqa: E402
...
from gsc.measurement_model import C_SI, D_M_flat, H0_to_SI, distance_modulus_from_D_L  # noqa: E402


TOOL = "phase4_triangle1_sn_bao_planck_thetastar"
TOOL_VERSION = "m157-v2"
SCHEMA = "phase4_triangle1_report_v1"
...
DEFAULT_PLANCK_PRIOR = "data/cmb/planck2018_distance_priors_chw2018_base_plikHM_TTTEEE_lowE.csv"
DEFAULT_Z_STAR = 1090.0
...
def _to_dense_cov(cov_like: Any, n: int) -> List[List[float]]:
    if n <= 0:
        raise JointError("covariance dimension must be positive")
    np = _optional_numpy()
    if np is not None and hasattr(cov_like, "shape"):  # pragma: no cover - numpy optional
        arr = np.asarray(cov_like, dtype=float)
        if arr.shape != (n, n):
            raise JointError(f"VECTOR_over_rd covariance shape mismatch: got {arr.shape}, expected {(n, n)}")
        return [[float(arr[i, j]) for j in range(n)] for i in range(n)]

    mat: List[List[float]] = []
    for row in cov_like:
        mat.append([float(v) for v in row])
    if len(mat) != n or any(len(row) != n for row in mat):
        raise JointError(f"VECTOR_over_rd covariance shape mismatch: got {(len(mat), len(mat[0]) if mat else 0)}, expected {(n, n)}")
    return mat


def _block_diag(mats: Sequence[Sequence[Sequence[float]]]) -> List[List[float]]:
    total = sum(len(m) for m in mats)
    out = [[0.0 for _ in range(total)] for _ in range(total)]
    offset = 0
    for mat in mats:
        n = len(mat)
        for i in range(n):
            if len(mat[i]) != n:
                raise JointError("covariance block must be square")
            for j in range(n):
                out[offset + i][offset + j] = float(mat[i][j])
        offset += n
    return out


def _bao_vector_observed(dataset: BAODataset) -> BAOVectorObserved:
    kinds: List[str] = []
    zs: List[float] = []
    ys: List[float] = []
    cov_blocks: List[List[List[float]]] = []
    for block in dataset.blocks:
        if isinstance(block, BAOBlock1D):
            kinds.append("DV")
            zs.append(float(block.z))
            ys.append(float(block.y))
            cov_blocks.append([[float(block.sigma) * float(block.sigma)]])
            continue

        if isinstance(block, BAOBlock2D):
            kinds.extend(("DM", "DH"))
            zs.extend((float(block.z), float(block.z)))
            ys.extend((float(block.y_dm), float(block.y_dh)))
            sig_dm = float(block.sigma_dm)
            sig_dh = float(block.sigma_dh)
            rho = float(block.rho_dm_dh)
            cov_blocks.append(
                [
                    [sig_dm * sig_dm, rho * sig_dm * sig_dh],
                    [rho * sig_dm * sig_dh, sig_dh * sig_dh],
                ]
            )
            continue

        if isinstance(block, BAOBlockND):
            n = len(block.y)
            if not (len(block.kinds) == len(block.zs) == n):
                raise JointError("BAOBlockND has inconsistent lengths")
            for kind, z, y in zip(block.kinds, block.zs, block.y):
                kk = str(kind).strip().upper()
                if kk not in ("DV", "DM", "DH"):
                    raise JointError(f"unsupported BAO kind in VECTOR_over_rd block: {kk}")
                kinds.append(kk)
                zs.append(float(z))
                ys.append(float(y))
            cov_blocks.append(_to_dense_cov(block.cov, n))
            continue

        raise JointError(f"unsupported BAO block type: {type(block).__name__}")

    if not ys:
        raise JointError("empty BAO observed vector")
    cov = _block_diag(cov_blocks)
    return BAOVectorObserved(kinds=kinds, zs=zs, y=ys, cov=cov)


def _load_planck_theta_prior(repo_root: Path, priors_csv_arg: str) -> ThetaPrior:
    priors_path = _resolve_path_from_repo(repo_root, str(priors_csv_arg))
    if not priors_path.is_file():
        raise UsageError(f"Planck prior CSV not found: {priors_path}")

    lA_mean: Optional[float] = None
    lA_sigma: Optional[float] = None
    with priors_path.open("r", encoding="utf-8", newline="") as fh:
        reader = csv.DictReader(fh)
        if reader.fieldnames is None:
            raise UsageError(f"missing CSV header in Planck prior file: {priors_path.name}")
        for row in reader:
            name = str(row.get("name", "")).strip()
            if name != "lA":
                continue
            try:
                lA_mean = float(str(row.get("value", "")).strip())
                lA_sigma = float(str(row.get("sigma", "")).strip())
            except Exception as exc:
                raise JointError(f"invalid lA row in Planck prior CSV: {priors_path.name}") from exc
            break

    if lA_mean is None or lA_sigma is None:
        raise JointError(f"Planck prior CSV does not contain required lA row: {priors_path.name}")
    if not (math.isfinite(lA_mean) and lA_mean > 0.0 and math.isfinite(lA_sigma) and lA_sigma > 0.0):
        raise JointError("Planck lA prior must be finite and positive")

    theta_mean = math.pi / lA_mean
    theta_sigma = math.pi * lA_sigma / (lA_mean * lA_mean)
    return ThetaPrior(
        lA_mean=float(lA_mean),
        lA_sigma=float(lA_sigma),
        theta_mean=float(theta_mean),
        theta_sigma=float(theta_sigma),
        relpath=_relative_or_basename(priors_path, repo_root),
        sha256=_sha256_file(priors_path),
    )


def _transform_bao_with_theta(
    *,
    y_over_rd: Sequence[float],
    cov_y: Sequence[Sequence[float]],
    theta_mean: float,
    theta_sigma: float,
) -> Tuple[List[float], List[List[float]]]:
    n = len(y_over_rd)
    if n <= 0:
        raise JointError("BAO transformed vector is empty")
    if len(cov_y) != n or any(len(row) != n for row in cov_y):
        raise JointError("BAO covariance shape mismatch for theta transform")
    obs = [float(theta_mean) * float(v) for v in y_over_rd]
    cov: List[List[float]] = [[0.0 for _ in range(n)] for _ in range(n)]
    ts2 = float(theta_sigma) * float(theta_sigma)
    tm2 = float(theta_mean) * float(theta_mean)
    y = [float(v) for v in y_over_rd]
    for i in range(n):
        for j in range(n):
            cov[i][j] = tm2 * float(cov_y[i][j]) + ts2 * y[i] * y[j]
    return obs, cov
...
def _valid_sha256_entry(value: Any) -> bool:
    if not isinstance(value, Mapping):
        return False
    sha = value.get("sha256")
    return isinstance(sha, str) and len(sha.strip()) == 64
...
def _save_png(fig: Any, out_path: Path) -> None:
    fig.savefig(
        ...
        dpi=140,
        metadata={
            "Software": "GSC phase4_triangle1_sn_bao_planck_thetastar",
...
def _render_markdown(payload: Mapping[str, Any]) -> str:
    lines: List[str] = []
    lines.append("# Triangle-1 SN+BAO+Planck theta* Posterior (Phase-4 M157)")
    lines.append("")
    lines.append("Deterministic joint artifact combining Pantheon+ SN, DESI BAO summary likelihood, and Planck acoustic-scale compressed prior.")
    lines.append("")
    ...
    theta = payload.get("planck_theta_prior", {}) if isinstance(payload.get("planck_theta_prior"), Mapping) else {}
    ...
    lines.append(f"- chi2_sn: `{float(best.get('chi2_sn', float('nan'))):.6f}`")
    lines.append(f"- chi2_bao_theta: `{float(best.get('chi2_bao_theta', float('nan'))):.6f}`")
    lines.append("")
    ...
    lines.append("## Compressed Planck Prior (CHW2018)")
    lines.append("")
    lines.append(f"- lA mean/sigma: `{float(theta.get('lA_mean', float('nan'))):.6e}` / `{float(theta.get('lA_sigma', float('nan'))):.6e}`")
    lines.append(
        f"- theta* mean/sigma (`theta*=pi/lA`): `{float(theta.get('theta_star_mean', float('nan'))):.12e}` / `{float(theta.get('theta_star_sigma', float('nan'))):.12e}`"
    )
    lines.append("")
    ...
    lines.append("- Triangle-1 closure assumption: `r_d ~ r_s`; BAO constraints are transformed with Planck `theta*` prior.")
    lines.append("- Planck term is compressed-prior only (not full CMB likelihood).")
    lines.append("- DR1 baseline for paper-grade path; DR2 BAO/cosmology products are robustness checks when public and supported by selected tooling.")
    lines.append("- This is a deterministic reviewer artifact, not a global cosmology fit claim.")
    lines.append("")
...
def _render_text(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"chi2_sn_best={float(best.get('chi2_sn', float('nan'))):.6f}",
        f"chi2_bao_theta_best={float(best.get('chi2_bao_theta', float('nan'))):.6f}",
        "report_json=TRIANGLE1_SN_BAO_PLANCK_REPORT.json",
    ]
    return "\n".join(lines) + "\n"
...
def _chi2_bao_theta(
    *,
    kinds: Sequence[str],
    zs: Sequence[float],
    obs_ratio: Sequence[float],
    solve_cov_ratio: Any,
    model: EpsilonMappedLCDMHistory,
    z_star: float,
    n: int,
) -> float:
    dm_star = D_M_flat(z=float(z_star), H_of_z=model.H, n=n)
    if not math.isfinite(dm_star) or dm_star <= 0.0:
        raise JointError("invalid D_M(z_star) while evaluating BAO-theta chi2")
    pred: List[float] = []
    for kind, z in zip(kinds, zs):
        kk = str(kind).upper()
        zz = float(z)
        if kk == "DV":
            d_val = D_V_flat(z=zz, model=model, n=n)
        elif kk == "DM":
            d_val = D_M_flat(z=zz, H_of_z=model.H, n=n)
        elif kk == "DH":
            d_val = D_H(z=zz, model=model)
        else:
            raise JointError(f"unknown BAO observable kind for Triangle-1: {kind!r}")
        pred.append(float(d_val) / float(dm_star))
    residual = [float(o) - float(p) for o, p in zip(obs_ratio, pred)]
    c_inv_r = solve_cov_ratio(residual)
    return _dot(residual, c_inv_r)


def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Deterministic Triangle-1 SN+BAO+Planck-theta* epsilon posterior.")
    ...
    ap.add_argument("--bao-desi-dr1-dataset", default=DEFAULT_BAO_DATASET)
    ap.add_argument("--bao-data-manifest", default=None)
    ap.add_argument("--planck-prior-csv", default=DEFAULT_PLANCK_PRIOR)
    ap.add_argument("--z-star", type=float, default=DEFAULT_Z_STAR)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    try:
        ...
        if run_mode == "paper_grade":
            ...
            if not args.bao_data_manifest:
                raise UsageError("--run-mode paper_grade requires --bao-data-manifest")
        ...
        z_star = _require_finite(float(args.z_star), name="z_star")
        if z_star <= 0.0:
            raise UsageError("--z-star must be > 0")
        ...
        bao_bundle = _load_bao_dataset(repo_root, str(args.bao_desi_dr1_dataset), toy_mode)
        bao_vector = _bao_vector_observed(bao_bundle.dataset)
        planck_prior = _load_planck_theta_prior(repo_root, str(args.planck_prior_csv))
        obs_ratio, cov_ratio = _transform_bao_with_theta(
            y_over_rd=bao_vector.y,
            cov_y=bao_vector.cov,
            theta_mean=planck_prior.theta_mean,
            theta_sigma=planck_prior.theta_sigma,
        )
        solve_cov_ratio = _make_cov_solver(cov_ratio)
        ndof_bao_theta_ref = int(len(obs_ratio))

        if run_mode == "paper_grade" and not toy_mode:
            if str(args.bao_desi_dr1_dataset).strip() == DEFAULT_BAO_DATASET:
                raise UsageError("--run-mode paper_grade requires converted full-cov DESI DR1 dataset input")
        ...
        bao_manifest_verification_mode: Optional[str] = None
        if bao_manifest_payload is not None and not toy_mode:
            ...
            files = bao_manifest_payload.get("files")
            if not isinstance(files, Mapping):
                raise UsageError("bao data manifest is missing 'files' object")

            dataset_key_candidates = (Path(str(args.bao_desi_dr1_dataset)).name, bao_bundle.relpath)
            dataset_entry: Optional[Mapping[str, Any]] = None
            ...
            if dataset_entry is not None and _valid_sha256_entry(dataset_entry):
                _verify_manifest_hash_with_candidates(
                    bao_manifest_payload,
                    key_candidates=dataset_key_candidates,
                    observed_sha256=bao_bundle.sha256,
                    label="bao_dataset",
                )
                bao_manifest_verification_mode = "dataset_sha256_match"
            else:
                mean_key = "desi_2024_gaussian_bao_ALL_GCcomb_mean.txt"
                cov_key = "desi_2024_gaussian_bao_ALL_GCcomb_cov.txt"
                if not (_valid_sha256_entry(files.get(mean_key)) and _valid_sha256_entry(files.get(cov_key))):
                    raise UsageError(
                        "bao data manifest must include either dataset sha256 entry or DR1 Gaussian mean/cov sha256 entries"
                    )
                bao_manifest_verification_mode = "dr1_gaussian_mean_cov_provenance"
        ...
        chi2_bao_theta_grid: List[List[float]] = []
        ...
        for i_om, omega_m in enumerate(omega_grid):
            ...
            for j_eps, z_gr_list in enumerate(z_gr_by_eps):
                ...
                chi2_bao_theta = _chi2_bao_theta(
                    kinds=bao_vector.kinds,
                    zs=bao_vector.zs,
                    obs_ratio=obs_ratio,
                    solve_cov_ratio=solve_cov_ratio,
                    model=model,
                    z_star=float(z_star),
                    n=int(args.integration_n),
                )
                if not math.isfinite(chi2_bao_theta):
                    raise JointError("non-finite BAO-theta chi2 encountered")

                chi2_joint = float(chi2_sn + chi2_bao_theta)
                ndof_joint = int(ndof_sn_ref + ndof_bao_theta_ref)

                row_sn.append(float(chi2_sn))
                row_bao_theta.append(float(chi2_bao_theta))
                ...
                rows.append(
                    RowResult(
                        ...
                        chi2_bao_theta=float(chi2_bao_theta),
                        ndof_joint=int(ndof_joint),
                        ndof_sn=int(ndof_sn_ref),
                        ndof_bao_theta=int(ndof_bao_theta_ref),
            ...
            chi2_bao_theta_grid.append(row_bao_theta)
                ...
                row_digest_lines.append(
                    ",".join(
                        (
                            ...
                            _fmt_e(chi2_bao_theta_grid[i_om][j_eps]),
        ...
        backend_1d = _write_png_1d(
            ...
            title="Triangle-1 epsilon_em marginal (SN+BAO+Planck theta*)",
        )
        backend_2d = _write_png_2d(
            ...
            title="Triangle-1 posterior in (omega_m, epsilon_em)",
        )
        ...
        assumptions = {
            ...
            "bao_planck_closure": "r_d≈r_s used to form BAO*theta* ratios D(z)/D_M(z*); covariance includes theta prior propagation",
            "planck_prior_mode": "compressed acoustic-scale prior only (lA -> theta*=pi/lA), not full CMB likelihood",
        ...
        inputs_payload: Dict[str, Any] = {
            ...
            "bao": {
                ...
                "n_observables": int(len(bao_vector.y)),
            },
            "planck_prior": {
                "dataset_relpath": str(planck_prior.relpath),
                "dataset_sha256": str(planck_prior.sha256),
                "name": "CHW2018_lA",
            },
            "grid": {
        ...
        if bao_manifest_payload is not None and bao_manifest_path is not None and bao_manifest_sha is not None:
            inputs_payload["bao_manifest"] = {
                ...
                "verification_mode": str(bao_manifest_verification_mode or "present_unverified"),
            }
        ...
        payload: Dict[str, Any] = {
            ...
            "planck_theta_prior": {
                "source_name": "CHW2018_lA",
                "lA_mean": float(planck_prior.lA_mean),
                "lA_sigma": float(planck_prior.lA_sigma),
                "theta_star_mean": float(planck_prior.theta_mean),
                "theta_star_sigma": float(planck_prior.theta_sigma),
            },
            ...
            "results": {
                "best_fit": {
                    ...
                    "chi2_bao_theta": float(best.chi2_bao_theta),
                    "ndof_joint": int(best.ndof_joint),
                    "ndof_sn": int(best.ndof_sn),
                    "ndof_bao_theta": int(best.ndof_bao_theta),
                    "omega_m": float(best.omega_m),
            ...
            "diagnostics": {
                ...
                "bao_theta_covariance_shape": [int(len(cov_ratio)), int(len(cov_ratio))],
            },
            "digests": {
        ...
        report_json = outdir / "TRIANGLE1_SN_BAO_PLANCK_REPORT.json"
        report_md = outdir / "TRIANGLE1_SN_BAO_PLANCK_REPORT.md"
        ...
        for rel, kind in (
            ...
            ("TRIANGLE1_SN_BAO_PLANCK_REPORT.md", "report_markdown"),
            ("TRIANGLE1_SN_BAO_PLANCK_REPORT.json", "report_json"),
        ):
            p = outdir / rel
            row: Dict[str, str] = {"filename": rel, "kind": kind}
            if p.is_file() and rel != "TRIANGLE1_SN_BAO_PLANCK_REPORT.json":
