...
> **Flagged — open problem 1 ([OPEN_PROBLEMS.md](../../OPEN_PROBLEMS.md)).** "Decisive" is withdrawn: §5.1 finds the BAO test indicative alone, and a joint CMB + BAO fit of the only coherent reading of this shift absorbs it into H₀ ([analyses/joint_fit.md](../../analyses/joint_fit.md)).
...
- A deterministic reproducibility stack that supports content-hashed, git-time-stamped pre-registered numerical predictions (GPG signing specified but not yet executed);
...
> **Superseded (v20.1).** The +0.42% H₀ bias comes from P1's heuristic convention, which holds the CMB acoustic angle fixed. The joint fit of the only coherent reading moves the acoustic angle too and absorbs the shift as a −0.9% change of H₀ ([analyses/joint_fit.md](../../analyses/joint_fit.md); OPEN_PROBLEMS.md, problem 4).
