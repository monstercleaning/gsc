...
We document the protocol, demonstrate the implementation on fifteen registered predictions (BAO ruler shift, 21cm Cosmic-Dawn signal, neutron-lifetime beam–trap test, CMB cosmic birefringence, strong-CP θ-bound, Kibble–Zurek defect spectrum, gravitational-wave-memory atomic-clock signature, Sandage–Loeb redshift drift, proton-electron mass-ratio constancy, TeV blazar arrival-time dispersion, the distance-duality relation, nuclear–electronic clock-ratio drift, gravitational-wave–electromagnetic distance duality, early-universe gravity,
and the galaxy acceleration scale), describe a self-verification layer that checks the documentation against the package, and discuss adoption considerations for other research programmes.
...
We implement these four components in the GSC framework's reproducibility stack and demonstrate the operational workflow end-to-end on fifteen registered cosmological predictions. The stack is licensed under MIT and available at the project repository.
...
We demonstrate the workflow on the fifteen registered predictions of the GSC framework. Detailed records are in `predictions/`, one folder per prediction; here we summarise the methodological aspects.
...
prediction rather than used to rescue P1. P15 records a prediction suggested by data the register then excludes
from its scoring: the rotation curves that suggested it cannot score it. Per-prediction details are in each entry's
`prediction.md`.
...
- A protection against the *choice* of which observations to register against (the choice of P1–P15 itself reflects researcher selection);
- Evidence of predictive success from retrodictive checks: nine of the fifteen entries were scored against data that were public when their pipelines were written, so their passes exercise the tooling rather than test a prediction made in advance;
