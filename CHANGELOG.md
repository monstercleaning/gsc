...
## [v12.7 cross-pipeline consistency correction] — September 2026

A fresh full review of the project, requested by the operator after a model change, targeted the one layer no previous pass had examined: **physical consistency between the prediction pipelines**, as opposed to the consistency of counts, hashes, schemas and prose that the v12.4–v12.6 machinery already enforces. It found a real contradiction, of the same weight as the v12.3 signing finding, and this cycle corrects it in full.

**The finding.** The shared parameter p = 6×10⁻⁴ played two incompatible roles under one symbol. In P1, P2, P4, P5 and P9 it is the T2 *metrology* exponent — a leading-order modulation σ(z)/σ(0) = (1+z)^{−p} of atomic units on a flat-ΛCDM background, which is the framework's actual claim (T1 conformally ΛCDM-equivalent, T2 a sub-percent effect). In P8 the same number was fed to `PowerLawHistory`, the v10.1 toy in which p is the *whole expansion law*, H(z) = H₀(1+z)^p — at p = 6×10⁻⁴ a coasting universe. Against the DESI DR1 BAO points bundled with this very package, that history is excluded at +7σ (D_M/r_d, z = 0.51) to +128σ (D_H/r_d, z = 2.33). Everything downstream of it was an artefact: the registered P8 table, Paper A's abstract-level "second decisive test" (positive drift at all z, sign flip vs ΛCDM), §5.2, its conclusion, framework §9.8, INDEX, QUICKSTART, the register README and the pre-registration table. The project's own archived Roadmap v2.8 §E.1 had already established that positive drift at z > 2 is impossible for standard matter content; the v12 layout lost that knowledge, and the v12.5 "single source of truth" refactor then wired P8 to the canonical constant without checking its role — cementing the conflation. The conflation itself dates to v12.2 (commit 137a379); the v12.5 step is ours and is recorded as such.

**The correction.** (1) New T2-consistent history `gsc.measurement_model.SigmaModulatedLCDMHistory`, H(z) = H_ΛCDM(z)·(1+z)^p, with the toy class carrying a warning docstring. (2) **P8 revision r2**: the pipeline (v0.2, schema `predictions_p8_pipeline_output_v2`) now uses the consistent history; the r1 output is *retained* as `pipeline_output.r1_superseded.json` (its SHA-256 recorded inside r2's `supersedes` block) and is reproducible through a loudly-named provenance option — nothing deleted, the claim withdrawn. Result: GSC's drift equals ΛCDM's to within 0.03 cm/s at every grid point with identical sign structure (both flip between z = 1.5 and 2.0); P8 is a ΛCDM-degenerate consistency test with no framework-specific discriminating power. The schema pins that: `all_same_sign_as_lcdm` const true, |Δv_GSC − Δv_ΛCDM| ≤ 0.1 cm/s per point. (3) Every prose surface corrected with the correction disclosed inline (Paper A abstract, §4.3 "on the symbol p", §5 intro, §5.2, discussion bullet, conclusion; framework list item 8, §9.8, §12.2.1 evaluability note; INDEX, QUICKSTART, register README, pre-registration table, risk register, measurement-model status note, frontier doc). (4) `gsc/canonical_params.py` documents the role of p (`CANONICAL_P_ROLE = "sigma_metrology_exponent"`). (5) Two new mechanical guards in CLAIMS.json: `registered-pipelines-never-use-coasting-toy-history` (no registered pipeline may instantiate `PowerLawHistory`) and `no-unqualified-drift-sign-flip-claim` (keeps the withdrawal enforced across all prose, as `no-unqualified-explained-anomaly` does for the v12.1 retraction). (6) Regression tests: the modulated history must stay sub-percent from ΛCDM with ΛCDM's drift sign structure, plus a negative control asserting the toy at canonical p *is* coasting — so the misuse remains detectable.

**What it means for the framework, stated plainly.** The late-time content beyond ΛCDM is now confined to the BAO metrology shift (P1) plus the four-sector exact-null package (P9, P11, P12, P13). The kill-condition's forward scope (P2, P8, P10, P12, P1-full) is left formally unchanged, but P8 can no longer contribute a framework-specific failure — a reduction in falsifiable forward content, recorded rather than re-tuned. Paper A's earlier statement of "two decisive tests" is withdrawn in the paper itself.

**Lesson recorded.** A single source of truth for a *number* is only as safe as the agreement on what the number *means*. Count, hash and schema checks cannot see two pipelines instantiating incompatible physics from one constant; the new guards encode this one case, and cross-pipeline physical consistency joins the standing review checklist (TEAM.md).
