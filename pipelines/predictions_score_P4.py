...
Reads:
  predictions/P04_cmb_birefringence/pipeline_output.json
  predictions/P04_cmb_birefringence/observed_data.json
...
Usage:
    ...
    python3 pipelines/predictions_score_P4.py --confidence 3
"""
...
PRED_DIR = REPO_ROOT / "predictions" / "P04_cmb_birefringence"
...
def render_scorecard(card: dict) -> str:
    ...
    parts.append(
        "\n## Reproduce\n\n"
        "```bash\n"
        "python3 pipelines/predictions_compute_P4.py\n"
        "python3 pipelines/predictions_score_P4.py\n"
        "```\n"
    )
    return "".join(parts)
