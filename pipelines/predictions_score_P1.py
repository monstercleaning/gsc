...
Reads:
  predictions/P01_bao_ruler_shift/pipeline_output.json
  predictions/P01_bao_ruler_shift/observed_data.json
...
This scorer was added to close a schema-gap loophole found in a hostile review (the loosened P1 schema upper bound was promised
to be enforced by a scorer that did not exist; this is that scorer).

Usage:
    python3 pipelines/predictions_score_P1.py
    python3 pipelines/predictions_score_P1.py --confidence 3
"""
...
PRED_DIR = REPO_ROOT / "predictions" / "P01_bao_ruler_shift"
...
def render_scorecard(card: dict) -> str:
    ...
    parts.append(
        ...
        "> Year-3 data, is itself already public since 2025-03). See `METHOD.md`\n"
    ...
    parts.append(
        "\n## Reproduce\n\n"
        "```bash\n"
        "python3 pipelines/predictions_compute_P1.py\n"
        "python3 pipelines/predictions_score_P1.py\n"
        "```\n"
    )
    return "".join(parts)
