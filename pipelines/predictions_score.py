...
Implements the scoring protocol documented in METHOD.md.

Usage:
    python3 pipelines/predictions_score.py P1
    python3 pipelines/predictions_score.py P1 --observed-data path/to/observed.json
    python3 pipelines/predictions_score.py --list

Status: SCAFFOLD — protocol defined, per-prediction scoring algorithms pending.

The scoring operation:

1. Loads the signed prediction at predictions/<ID>/prediction.md.
...
REGISTER_DIR = REPO_ROOT / "predictions"
...
def resolve_prediction_dir(prediction_id: str) -> Path | None:
    """Accept either full directory name (P01_bao_ruler_shift) or short ID (P1)."""
...
def score(prediction_id: str, observed_path: Path | None) -> int:
    ...
    sys.stdout.write(
        "  Scoring algorithms are defined in each predictions/<ID>/prediction.md.\n"
    )
