...
Reads:
  predictions/P03_neutron_lifetime/pipeline_output.json   (predictions)
  predictions/P03_neutron_lifetime/observed_data.json     (PDG values)
...
Usage:
    python3 pipelines/predictions_score_P3.py
    python3 pipelines/predictions_score_P3.py --observed predictions/P03_neutron_lifetime/observed_data.json
    python3 pipelines/predictions_score_P3.py --confidence 3
"""
...
PRED_DIR = REPO_ROOT / "predictions" / "P03_neutron_lifetime"
...
def render_scorecard(card: dict) -> str:
    ...
    parts.append(
        "\n## Reproduce\n\n"
        "```bash\n"
        "python3 pipelines/predictions_compute_P3.py\n"
        "python3 pipelines/predictions_score_P3.py\n"
        "```\n"
    )
    return "".join(parts)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.pipeline.is_file():
        ...
        sys.stderr.write("  run: python3 pipelines/predictions_compute_P3.py\n")
        return 2
