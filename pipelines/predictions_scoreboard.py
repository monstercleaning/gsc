...
Usage:
    python3 pipelines/predictions_scoreboard.py
    python3 pipelines/predictions_scoreboard.py --format json
    python3 pipelines/predictions_scoreboard.py --format markdown > scoreboard.md

Reads each predictions/<ID>/prediction.md, extracts front-matter,
...
def render_text(rows: list[dict]) -> str:
    if not rows:
        return "(no predictions found in predictions/)\n"
    out: list[str] = []
