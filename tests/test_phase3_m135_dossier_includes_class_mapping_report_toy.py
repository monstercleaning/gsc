...
def _write_toy_scan_jsonl(path: Path) -> None:
    row = {
        ...
        "status": "ok",
        "plan_point_id": "m135_dossier_mapping_0001",
        "point_index": 0,
...
class TestPhase3M135DossierIncludesClassMappingReportToy(unittest.TestCase):
    def test_dossier_includes_class_mapping_report(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            cand = cand_dirs[0]
            self.assertTrue((cand / "class_mapping" / "CLASS_MAPPING_REPORT.json").is_file())

            manifest = json.loads((dossier_dir / "DOSSIER_MANIFEST.json").read_text(encoding="utf-8"))
            candidates = manifest.get("candidates")
            self.assertIsInstance(candidates, list)
            self.assertEqual(len(candidates), 1)
            subtools = candidates[0].get("subtools")
            self.assertIsInstance(subtools, dict)
            class_mapping = subtools.get("class_mapping")
            self.assertIsInstance(class_mapping, dict)
            self.assertIs(class_mapping.get("ok"), True)
