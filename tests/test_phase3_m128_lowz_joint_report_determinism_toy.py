...
class TestPhase3M128LowzJointReportDeterminismToy(unittest.TestCase):
    def test_deterministic_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            proc_a = subprocess.run([*base_cmd, "--outdir", str(out_a)], cwd=str(ROOT), text=True, capture_output=True)
            self.assertEqual(proc_a.returncode, 0, msg=(proc_a.stdout or "") + (proc_a.stderr or ""))

            proc_b = subprocess.run([*base_cmd, "--outdir", str(out_b)], cwd=str(ROOT), text=True, capture_output=True)
            self.assertEqual(proc_b.returncode, 0, msg=(proc_b.stdout or "") + (proc_b.stderr or ""))

            files = ("LOWZ_JOINT_REPORT.json", "LOWZ_JOINT_REPORT.md")
            for name in files:
            ...
            self.assertEqual(payload.get("schema"), "phase3_sigmatensor_lowz_joint_report_v1")
