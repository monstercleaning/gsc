...
| Redshift drift sign at high redshift (`z >= 2`) (supporting no-go diagnostic) | A robust measurement of **negative** `dz/dt0` in the quasar range (e.g. `z ~ 2-5`) | *(v12.7)* The T2-consistent history (ΛCDM × metrology modulation, P8 r2) reproduces ΛCDM's drift to 0.03 cm/s with the same sign structure; only the retired v10.1 *toy* histories predicted positive drift in the quasar range, and Roadmap v2.8 §E.1 showed that is impossible for standard matter. A negative high-z drift is therefore the *expected* outcome, not a kill; a drift inconsistent with ΛCDM-class kinematics would falsify T1/T2 and ΛCDM alike. | Paper (historical framing): `GSC_Framework_v10_1_FINAL.md` ("8. Redshift drift (historical; deprecated as primary)"); code: `gsc/measurement_model.py` (`z_dot_sandage_loeb`); tests: `tests/test_history_gsc.py`; helper: `scripts/redshift_drift_table.py`; diagnostic: `scripts/phase4_sigmatensor_drift_sign_diagnostic.py` |
