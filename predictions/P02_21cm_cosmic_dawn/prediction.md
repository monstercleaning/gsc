...
> **Editorial flag (added at the reorganization; the registered statement below is unchanged):** the pipeline's amplification factor K_σ = 50 is not derived from any coupling; in the only coherent reading of the canonical parameter the effect at z = 17 is about 0.1%, not 8.7%. See OPEN_PROBLEMS.md, problem 3.
...
> **Editorial flag (added at the reorganization; the registered text below is unchanged):** the modules this section names were planned at registration and never built. The registered output comes from `pipelines/predictions_compute_P2.py`, a simplified parametric implementation. See OPEN_PROBLEMS.md, problem 6.
...
4. New script `pipelines/predictions_compute_P2.py` — computes δT_b(ν) for the registered σ(t) ansatz, outputs to `pipeline_output.json`.
