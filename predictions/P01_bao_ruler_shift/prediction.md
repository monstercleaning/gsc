...
> **Editorial flag (added at the reorganization; the registered text below is unchanged):** the modules this section names were planned at registration and never built. The registered output comes from `pipelines/predictions_compute_P1.py`, a simplified parametric implementation. See OPEN_PROBLEMS.md, problem 6.
...
2. New script `pipelines/predictions_compute_P1.py` that:
...
3. Scoring script `pipelines/predictions_score_P1.py` that:
...
> **Editorial flag (v20.1; the registered text below is unchanged):** a joint fit of the only coherent reading of
> this shift to the Planck 2018 CMB distance priors and the DESI DR2 BAO data
...
3. Author runs `pipelines/predictions_sign.py P1` which:
