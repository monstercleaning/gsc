...
**T4** — depends on the σ(x,t) field-theoretic extension (Section 6 of THEORY.md). Failure of this prediction does not affect T1–T3.

## Pipeline

> **Editorial flag (added at the reorganization; the registered text below is unchanged):** the modules this section names were planned at registration and never built. The registered output comes from `pipelines/predictions_compute_P3.py`, a simplified parametric implementation. See OPEN_PROBLEMS.md, problem 6.
...
3. New script `pipelines/predictions_compute_P3.py` — produces (τ_n^beam, τ_n^trap) for the registered σ(x,t) ansatz.
