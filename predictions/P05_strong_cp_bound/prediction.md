...
> **Editorial flag (added at the reorganization; the registered text below is unchanged):** the modules this section names were planned at registration and never built. The registered output comes from `pipelines/predictions_compute_P5.py`, a simplified parametric implementation. See OPEN_PROBLEMS.md, problem 6.
...
2. Script `pipelines/predictions_compute_P5.py` — produces θ_eff(z=0) and the trajectory θ_eff(z) for z ∈ [0, 1100], with uncertainty band.
