...
* Data model: DESI DR1 per-tracer central values REPLACED by the noiseless
  ...
  (data/desi_dr1_bao_baseline.csv).
...
Usage:
    python3 analyses/w0wa_rd_shift_diagnostic.py [--fast] [--output PATH]
"""
...
# Source: data/desi_dr1_bao_baseline.csv (DESI DR1 compact set).
DESI_DR1_BLOCKS = [
...
def registered_shift() -> float:
    ...
    p1 = json.loads((REPO_ROOT / "predictions" / "P01_bao_ruler_shift"
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--output", default=str(REPO_ROOT / "analyses" / "w0wa_rd_shift_diagnostic.json"))
    ...
    result = {
        ...
        "inputs": {
            ...
            "delta_source": "predictions/P01_bao_ruler_shift/pipeline_output.json (powerlaw delta_rs_relative)",
